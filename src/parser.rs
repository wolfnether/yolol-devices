...
/*impl<'a> IntoIterator for YamlParser<'a> {
    type Item = <YamlIterator<'a> as Iterator>::Item;

    type IntoIter = YamlIterator<'a>;

    fn into_iter(self) -> Self::IntoIter {
        Self::IntoIter {
            iterator: self.parser.into_iter(),
        }
    }
}

impl<'a> Iterator for YamlIterator<'a> {
    type Item = Option<YamlElement>;

    fn next(&mut self) -> Option<Self::Item> {
        while let Some(Ok(i)) = self.iterator.next() {
            println!("{:?}", i);
            match i {
                libyaml::Event::StreamStart { .. } => (),
                libyaml::Event::StreamEnd => todo!(),
                libyaml::Event::DocumentStart {
                    version,
                    tags,
                    implicit,
                } => todo!(),
                libyaml::Event::DocumentEnd { implicit } => todo!(),
                libyaml::Event::Alias { anchor } => todo!(),
                libyaml::Event::Scalar {
                    anchor,
                    tag,
                    value,
                    plain_implicit,
                ...
                libyaml::Event::SequenceStart {
                    anchor,
                    tag,
                    implicit,
                    style,
                } => todo!(),
                libyaml::Event::SequenceEnd => todo!(),
                libyaml::Event::MappingStart {
                    anchor,
                    tag,
                    implicit,
                    style,
                } => todo!(),
                libyaml::Event::MappingEnd => todo!(),
            }
        }
        None
    }
